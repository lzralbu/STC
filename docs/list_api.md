...
int main(void) {
    list_i L1 = c_init(list_i, {1, 2, 3, 4, 5});
    list_i L2 = c_init(list_i, {10, 20, 30, 40, 50});

    list_i_iter i = list_i_advance(list_i_begin(&L1), 2);
    list_i_iter j1 = list_i_advance(list_i_begin(&L2), 2), j2 = list_i_advance(j1, 2);

    list_i_splice_range(&L1, i, &L2, j1, j2);

    c_foreach (i, list_i, L1)
        printf(" %d", *i.ref); puts("");
    c_foreach (i, list_i, L2)
        printf(" %d", *i.ref); puts("");
